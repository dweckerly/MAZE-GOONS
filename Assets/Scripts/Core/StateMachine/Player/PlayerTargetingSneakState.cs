...
public class PlayerTargetingSneakState : PlayerLocomotiveState
{
    ...
    public PlayerTargetingSneakState(PlayerStateMachine _stateMachine) : base(_stateMachine) { }
    ...
    private void UpdateAnimator(float deltaTime)
    {
        ...
        else
        {
            ...
            stateMachine.animator.SetFloat(TargetingForward, value / 2, 0.1f, deltaTime);
        }
        if (stateMachine.InputReader.MovementValue.x == 0) stateMachine.animator.SetFloat(TargetingRight, 0, 0.1f, deltaTime);
        else
        {
            float value = stateMachine.InputReader.MovementValue.x > 0 ? 1f : -1f;
            stateMachine.animator.SetFloat(TargetingRight, value / 2, 0.1f, deltaTime);
        }
    }
